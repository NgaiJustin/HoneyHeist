...
public class RagdollModel extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		anchor.set(getBody(PART_LEFT_ARM).getWorldPoint(getOffsetVector(PART_LEFT_FOREARM).scl(.5f)));
		addJoint(world, getBody(PART_LEFT_FOREARM), getBody(PART_LEFT_ARM), anchor);

		anchor.set(getBody(PART_RIGHT_ARM).getWorldPoint(getOffsetVector(PART_RIGHT_FOREARM).scl(-.5f)));
		addJoint(world, getBody(PART_RIGHT_FOREARM), getBody(PART_RIGHT_ARM), anchor);

		temp = getOffsetVector(PART_LEFT_ARM);
		BoxObstacle mainBody = (BoxObstacle)bodies.get(PART_BODY);
		temp.x = mainBody.getWidth() * -.5f;
		anchor.set(getBody(PART_BODY).getWorldPoint(temp));
		addJoint(world, getBody(PART_LEFT_ARM), getBody(PART_BODY), anchor);

		temp = getOffsetVector(PART_RIGHT_ARM);
		temp.x = mainBody.getWidth() * .5f;
		anchor.set(getBody(PART_BODY).getWorldPoint(temp));
		addJoint(world, getBody(PART_RIGHT_ARM), getBody(PART_BODY), anchor);

		temp = getOffsetVector(PART_HEAD);
		temp.y = mainBody.getHeight() * .5f;
		anchor.set(getBody(PART_BODY).getWorldPoint(temp));
		addJoint(world, getBody(PART_HEAD), getBody(PART_BODY), anchor);

		temp = getOffsetVector(PART_LEFT_THIGH);
		temp.y = mainBody.getHeight() * -.5f;
		anchor.set(getBody(PART_BODY).getWorldPoint(temp));
		addJoint(world, getBody(PART_LEFT_THIGH), getBody(PART_BODY), anchor);

		temp = getOffsetVector(PART_RIGHT_THIGH);
		temp.y = mainBody.getHeight() * -.5f;
		anchor.set(getBody(PART_BODY).getWorldPoint(temp));
		addJoint(world, getBody(PART_RIGHT_THIGH), getBody(PART_BODY), anchor);

		temp = getOffsetVector(PART_LEFT_SHIN).scl(.5f);
		anchor.set(getBody(PART_LEFT_THIGH).getWorldPoint(temp));
		addJoint(world, getBody(PART_LEFT_SHIN), getBody(PART_LEFT_THIGH), anchor);

		temp = getOffsetVector(PART_RIGHT_SHIN).scl(.5f);
		anchor.set(getBody(PART_RIGHT_THIGH).getWorldPoint(temp));
		addJoint(world, getBody(PART_RIGHT_SHIN), getBody(PART_RIGHT_THIGH), anchor);
	...
	/**
	 * Returns the offset vector of the body part.
	 * @param bodyPart the body part.
	 * @return the offset vector.
	 */
	private Vector2 getOffsetVector(int bodyPart) {
		float x = data.get( BODY_PARTS[partToAsset( bodyPart )] ).getFloat( 0 );
		float y = data.get( BODY_PARTS[partToAsset( bodyPart )] ).getFloat( 1 );
		if (partOnLeft(bodyPart)) {
			x = -x;
		}
		temp.set(x, y);
		return temp;
	 ...
	 */
	private Body getBody(int bodyPart) {
		return bodies.get(bodyPart).getBody();
	}

	/**
	 * Adds a joint between two body parts at the specified anchor.
	 * @param world the world which the bodies are in.
	 * @param bodyA the first body
	 * @param bodyB the second body
	 * @param anchor the position of the joint in world coordinates.
	 */
	private void addJoint(World world, Body bodyA, Body bodyB, Vector2 anchor) {
		RevoluteJointDef jointDef = new RevoluteJointDef();
		jointDef.initialize(bodyA, bodyB, anchor);
		jointDef.lowerAngle = -45 * 0.0174533f;
		jointDef.upperAngle = 45 * 0.0174533f;
